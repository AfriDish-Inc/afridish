    ...
    <!-- <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            @if(auth()->user()->user_type == "A")
               <a class="btn btn-success" href="{{ route("admin.vendor-category.create") }}">
             @elseif(auth()->user()->user_type == "V")   
               <a class="btn btn-success" href="{{ route("vendor.vendor-category.create") }}">
            @endif
                ...
                <tbody>
                    @foreach($categories as $key => $category)
                        <tr data-entry-id="{{ $category->id }}">
                           ...
                           <!--  <td>
                                @if(auth()->user()->user_type == "A")
                                   <a class="btn btn-xs btn-info" href="{{ route('admin.vendor-category.edit', $category->id) }}">
                                 @elseif(auth()->user()->user_type == "V")   
                                   <a class="btn btn-xs btn-info" href="{{ route('vendor.vendor-category.edit', $category->id) }}">
                                @endif
                                 ...
                                 @if(auth()->user()->user_type == "A")
                                   <form action="{{ route('admin.vendor-category.destroy', $category->id) }}" method="POST" onsubmit="return confirm('If you want delete ! All product removed  this category!');" style="display: inline-block;">
                                 @elseif(auth()->user()->user_type == "V")   
                                   <form action="{{ route('vendor.vendor-category.destroy', $category->id) }}" method="POST" onsubmit="return confirm('If you want delete ! All product removed  this category!');" style="display: inline-block;">
                                @endif
    ...
    function updateProductFeatures(id,status)
    {
        ...
        if (confirm('{{ trans('global.areYouSure') }}')) 
        {
            $.ajax({
                method: 'POST',
                url: 'updateFeatures',
                data: { product_id: id, status: status, model: 'vendor_categories' },
                success: function (data){
