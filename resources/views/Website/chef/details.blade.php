  ...
  <div class="row">
      <div class="col-md-12">
          <form class="vendor-details">
              <div class="name-icon">
                  <img src="{{ asset('upload/images/')}}/{{$chef->profile_picture}}">
                  <div class="v-name">
                      <label>Chef Name</label>
                      <input type="text" value="{{$chef->name}}">
                  </div>
                </div>
                <div class="under-flex">
                  <div class="v-name">
                    <label>Chef Email</label>
                  <input type="email" value="{{$chef->email}}">
                </div>
                <div class="v-name">
                    <label>Category</label>
                  <input type="text" value="{{optional(App\Models\VendorCategory::where('id',$chef->vendor_category_id)->first())->category_name}}">
                </div>
              </div>
                @if($chef->vendor_category_id == 1)
                <div class="v-name">
                    <label>Address</label>
                    <textarea rows="4" cols="50">{{$chef->address}}</textarea>
                </div>
                @endif
            </form>
          </div>
