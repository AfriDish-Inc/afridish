                ...
                if (Auth::user()) {
                    $products = Product::selectRaw("*,
                         ...
                         * cos( radians( longitude ) - radians(?)
                         ...
                         ) AS distance", [Auth::user()->latitude, Auth::user()->longitude, Auth::user()->latitude])->where('category_id', '=', 18)->having('distance', '<=', 10)->get();
                }else{
    ...
    public function productData(Request $request){
                ...
                if (Auth::user()) {
                    $data = Product::selectRaw("*,
                         ...
                         * cos( radians( longitude ) - radians(?)
                         ...
                         sin( radians( latitude ) ) )
                         ) AS distance", [Auth::user()->latitude, Auth::user()->longitude, Auth::user()->latitude])->where('category_id', '=', 18)->having('distance', '<=', 10)->latest()->get();
                    }else{
        ...
        if(Auth::user()){
            foreach ($data as $key => $featurevalue) {
                ...
                if(count($productReview) > 0){
                    $reviewCount = count($productReview);
                    $ratingsum = ProductReview::where('product_id',$featurevalue->id)->sum('rating');
                    $featurevalue->rating_count = $ratingsum/$reviewCount;
                 }else{
    ...
    public function addcart(Request $request){
      ...
      if (Auth::user()) {
            $validator = Validator::make($request->all(), [
             ...
             if($cartdata->vendor_id == $productdata->provider_id){
                ...
                $olddatas = Cart::where('user_id',Auth::user()->id)->delete();
                if (($productdata->quantity) >= $request->product_quantity) {
                    $cart = new Cart;
                    $cart->user_id = Auth::user()->id;            
                    $cart->product_id = $request->product_id;
